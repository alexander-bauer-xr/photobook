...
export type Item = {
  ...
  rotation?: number;
  auto?: boolean;
  _iw?: number; _ih?: number; _error?: boolean;
};
...
const getSrc = (it: Item) => (it as any).web || (it as any).webSrc || it.src || '';
...
export default function SlotView({
  ...
  const { slotLeft, slotTop, slotW, slotH, contentW, contentH, innerPad } = snapshot;

  // Size & math
  const iw = item._iw || 0, ih = item._ih || 0;
  ...
  return (
    ...
    `,
            ...
            onLoad={(ev) => {
              ...
              if (iwNat > 0 && ihNat > 0) {
                onUpdateItem((prev) => ({ ...prev, _iw: iwNat, _ih: ihNat, _error: false }));
              }
            }}
            onError={() => onUpdateItem(prev => ({ ...prev, _error: true }))}
            onMouseDown={(e) => {
              if (!(iw > 0 && ih > 0)) return;
              e.preventDefault();
              ...
              const startPan = alignOffsetToPanPx(startAlign, startOffset, math.overflowX, math.overflowY, contentW, contentH);

              const elImg = e.currentTarget as HTMLImageElement;
              const prevCursor = elImg.style.cursor; elImg.style.cursor = 'grabbing';
              ...
              function onMove(ev: MouseEvent) {
                ...
                raf = requestAnimationFrame(() => {
                  ...
                  const solved = solveAlignOffset(desiredPanX, desiredPanY, math.overflowX, math.overflowY, startOffset, contentW, contentH);
                  onUpdateItem(prev => ({ ...prev, align: solved.align, offset: solved.offset, auto: false }));
                });
              }
              function onUp() {
      ...
      <div className="absolute bottom-2 left-2 bg-white/85 rounded px-2 py-1 shadow flex items-center gap-2">
        ...
        <ZoomSlider
          ...
          min={0.05}
          max={6}
          onChange={(v) => onUpdateItem(prev => ({ ...prev, zoom: v, auto: false }))}
        />
      ...
      <div className="absolute top-2 left-2 bg-white/85 rounded px-2 py-1 shadow flex items-center gap-1">
        <button
          ...
          onClick={() => { onUpdateItem(prev => ({ ...prev, rotation: (((prev.rotation || 0) - 90) % 360 + 360) % 360, auto: false })); onCommit(); }}
          title="⟲ 90° (Shift+R)"
        ...
        <button
          className="px-2 py-1 text-xs bg-neutral-200 rounded hover:bg-neutral-300"
          onClick={() => { onUpdateItem(prev => ({ ...prev, fit: (prev.fit || 'cover') === 'cover' ? 'contain' : 'cover', auto: false })); onCommit(); }}
          title="Toggle fit (F)"
        ...
        <button
          className="px-2 py-1 text-xs bg-neutral-200 rounded hover:bg-neutral-300"
          onClick={() => { onUpdateItem(prev => ({ ...prev, align: { x: 0, y: 0 }, offset: { x: 0, y: 0 }, zoom: 1, rotation: 0, auto: false })); onCommit(); }}
          title="Reset (0 setzt Zoom)"
          aria-label="Reset"
        >Reset</button>
        <button
          className="px-2 py-1 text-xs bg-neutral-200 rounded hover:bg-neutral-300"
          onClick={() => { onUpdateItem(prev => ({ ...prev, auto: true })); onCommit(); }}
      ...
      {showHud && (
        <div className="absolute top-2 right-2 bg-white/85 rounded px-2 py-1 shadow text-[10px] leading-tight text-neutral-700">
          ...
          <div>align: {(item.align?.x ?? 0).toFixed(2)},{(item.align?.y ?? 0).toFixed(2)}</div>
          <div>offset: {(item.offset?.x ?? 0).toFixed(3)},{(item.offset?.y ?? 0).toFixed(3)}</div>
          <div>ovX/ovY: {math.overflowX.toFixed(1)}/{math.overflowY.toFixed(1)}</div>
        </div>
      )}
    </div>
  );
